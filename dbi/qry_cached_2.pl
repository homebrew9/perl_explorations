...
sub age_by_id {
    ...
    my ($dbh, $id) = @_;
    # The second way to cache prepared statements is to use "prepare_cached($sth, \%attr, $if_active)" instead of "prepare($sth, \%attr)".
    # $if_active = 0: A warning will be generated and finish() will be caled on $sth before it's returned. The default.
    #            = 1: finish() will be called on $sth, but the warning is suppressed
    #            = 2: disables any checking
    #            = 3: existing $sth will be removed from cache and a new $sth prepared and cached in its place.
    my $sth = $dbh->prepare_cached('SELECT age from people WHERE id = ?', {}, 1) or die "Couldn't prepare statement: ".$dbh->errstr;
