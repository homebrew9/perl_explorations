...
# One way to cache the prepare statement is by:
# (a) declaring the stmt handle $sth outside of the subroutine; that way it persists after the function call and
# (b) putting the prepare code in a conditional block. It's only executed if $sth does not yet have a value.
my $sth;
...
sub age_by_id {
    ...
    my ($dbh, $id) = @_;
    if (! defined $sth) {
        $sth = $dbh->prepare('SELECT age from people WHERE id = ?') or die "Couldn't prepare statement: ".$dbh->errstr;
    }
