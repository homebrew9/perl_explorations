...
sub new_employee {
    ...
    # No changes should be made to the database until we issue a commit. $success will be true if both queries
    # succeeded, false otherwise. If the queries succeeded, we commit the transaction; otherwise, we roll it back,
    # canceling all our changes. C-style Logical And (&&) returns true only if both operands are true, false otherwise.
    my $success = 1;
...
my $dbh = DBI->connect('DBI:Oracle:'.$db, $user, $password, {AutoCommit => 0}) or die "Couldn't connect to database: ".DBI->errstr;
print "\n";
